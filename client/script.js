...
sayHelloButton.addEventListener('mouseover',bkgColor)
...
sayHelloButton.addEventListener('mouseout',backToNorm)
...
const ohMy = () => {
   let request = axios.get('http://localhost:3000/animals')
    request.then(result => {
...
const repeatMyParam = () => {
    let request = axios.get('http://localhost:3000/repeat/Lions')
    request.then(result => console.log(result.data))
...
// CODE HERE 
const createFood = () => {
    let foodInput = document.getElementById('input-field').value
    let body = {
        newFood: foodInput
    }
